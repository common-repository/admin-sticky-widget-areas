...
add_action( 'admin_enqueue_scripts', 'aswa_load_stylesheet' );
/**
 * Load style.css on the WordPress widgets page in the backend.
 */
function aswa_load_stylesheet( $hook ) {
	// if this is not the Appearance > Widgets page, abort.
	if ( 'widgets.php' !== $hook ) {
		return;
	}

	wp_enqueue_style( 'admin-sticky-widget-areas-widgets-page-styles', plugin_dir_url( __FILE__ ) . 'assets/css/style.css' );
}
